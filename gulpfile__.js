...
function source() {
  return project.splitSource()
    ...
    .pipe(project.rejoin('SOURCES')); // Call rejoin when you're finished
}

// This source task will process dev build
function devSource() {
  return project.splitSource()
    .pipe(debug({title:"devSource:"}))
    //.pipe(gulpif('**/*.js', babel()))
    .pipe(project.rejoin('SOURCES')); // Call rejoin when you're finished
}
...
function dependencies() {
  return project.splitDependencies()
    ...
    //.pipe(gulpif('**/*.js', uglify()))
    .pipe(project.rejoin('DEPENDENCIES'));
}
...
gulp.task('app', gulp.series([gulp.parallel(['app-nodemon', 'watch-element-changes'])]));
