  ...
  _onRelModelsReceived(e) {
      ...
      if (responseContent) {
          ...
          for(let rel of this._relationshipEntityTypeMappings){
              if(relationshipTypeNames.indexOf(rel.relationshipTypeName) > -1 && this.excludeItems.indexOf(rel.relationshipTypeName) == -1){
                  let relEntitytypes = rel.relatedEntityTypes;
                  for(let relEntityType of relEntitytypes){
                      let relationshipMapping = {relationshipTypeName: rel.relationshipTypeName,
