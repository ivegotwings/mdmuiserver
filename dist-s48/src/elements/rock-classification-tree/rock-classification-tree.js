...
`;
  ...
  static get observers() {
      return [
          '_onSelectedItemsChange(_selectedItems.*, _selectedItem)',
          '_onRootNodeChange(rootNode)'
      ];
  }
  ...
  _onRootNodeGetResponse(e, detail) {
      ...
      if (_.isEmpty(entities) &&
          !DataHelper.isValidObjectPath(entities[0], "data.attributes.externalName.values.0.value")) {
          this.logError("Classification entity/extenalName missing for the save process");
          return;
      }

      this.rootNodeExternalName = entities[0].data.attributes.externalName.values[0].value;
  }
