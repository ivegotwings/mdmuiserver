...
DataTransformHelper.transformDataToLocaleCoalescedData = async function (data, request) {
    if (!_.isEmpty(data) && !_.isEmpty(data.entities)) {
        ...
        if (request && request.params && request.params.query && request.params.query.valueContexts && request.params.query.valueContexts.length > 0) {
            ...
            // if no fallback locales defined for current locale and current loclae is not default locale,
            // default locale will be considered as fallback locale to current locale
            if (_.isEmpty(fallBackLocales) && valContext.locale !== defaultLocale) {
                fallBackLocales = [
                    {
                        "name": defaultLocale
                    }
