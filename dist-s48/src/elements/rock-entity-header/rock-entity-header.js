...
`;
  ...
  _onSaveHeaderAttribute() {
      if(DataHelper.isValidObjectPath(this._headerAttributesGetResponse, 'content.entities.0.data.attributes')) {
          ...
          if(classificationTree) {
              ...
              if (this._headerAttributeModels[attributeForSave].displayType === "path") {
                  let pathTypeAttributeValues;
                  pathTypeAttributeValues = this._getFormattedPathTypeObject(classificationPaths);
                  let isDelete = false;
  ...
  _getFormattedPathTypeObject(classificationPaths) {
      let pathTypeAttributeValues = {
          values:[]
      }
