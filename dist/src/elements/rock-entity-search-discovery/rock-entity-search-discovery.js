                            ...
                            <div class="entityTypeFilterContainer m-r-10">
                                ...
                                <template is="dom-if" if="[[_entityTypeFilterPopover]]">
                                    <pebble-popover id="entityTypeFilterPopover" for="entityTypeFilterButton" no-overlap="" vertical-align="auto" horizontal-align="auto">
                                        <rock-entity-type-model-lov id="entityTypeModelLov" settings="[[_getComponentSettings('rock-entity-type-model-lov')]]" domain="[[domain]]" select-all="true" allowed-entity-types="[[allowedEntityTypes]]" selected-items="{{_selectedEntityTypes}}" show-action-buttons="">
                                ...
                                <template is="dom-if" if="[[_getVisibility('rock-query-builder')]]">
                                    <div class="advancedsearch">
                                        ...
                                        <template is="dom-if" if="[[_queryBuilderEnabled]]">
                                            <rock-query-builder id="queryBuilder" domain="[[domain]]" context-data="[[contextData]]" allowed-entity-types="{{allowedEntityTypes}}" selected-entity-types="{{_currentEntityTypes}}" input-query-string="{{inputQueryString}}" attribute-grid-data="{{attributeGridData}}" relationship-name="{{relationshipName}}" workflow-criterion="[[_workflowCriterion]]" relationships-data="[[_relationshipsData]]" rel-entity-data="[[_relEntityData]]" search-query="{{searchQuery}}" lov-settings="[[_getComponentSettings('rock-entity-type-model-lov')]]" entity-type-filter-text="[[entityTypeFilterText]]" settings="[[_getComponentSettings('rock-query-builder')]]" relationship-exists-search-criterion-data="[" [relationshipexistssearchcriteriondata]]="">
                                            </rock-query-builder>
                                        </template>
                                    </div>
  ...
  _configDataChanged() {
      if (!DataHelper.isEmptyObject(this.configData)) {
          if (this.configData && this.configData.componentSettings && this.configData.componentSettings["rock-entity-type-model-lov"]) {
              if (this.configData.componentSettings["rock-entity-type-model-lov"].title) {
                  this.entityTypeFilterText = this.configData.componentSettings["rock-entity-type-model-lov"].title;
              }
          }
