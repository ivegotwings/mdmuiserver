                                                            ...
                                                            <div slot="step-details" class="step-details">
                                                                ...
                                                                <template is="dom-if" if="[[_stepHasDetails(item.stepDetails, item.status)]]">
                                                                    <div id="action-details">
                                                                        ...
                                                                        <a hidden="" class="view-changes" href="#">View all changes »</a>
                                                                    </div>
                                                                </template>
                                                                <template is="dom-if" if="[[_isCommentsEnabled(item)]]">
                                                                    ...
                                                                    <div id="step-actions">
                                                                        <template is="dom-repeat" items="[[item.stepDetails.actions]]" as="button">
                                                                            <pebble-button disabled="[[readonly]]" class\$="[[_getActionButtonClass(button)]]" raised="" icon="[[button.actionIcon]]" button-text="[[button.action]]" on-tap="_onStepActionTap" data="[[_getDataforButton(button, item.id, _workflowstepperData.workflowId)]]"></pebble-button>
                                                                        </template>
                                                                    </div>
                                                                </template>
                                                            </div>
  ...
  _onEventsGetResponse (e) {
      ...
      if (workflowEventsResponse && workflowEventsResponse.response && workflowEventsResponse.response.events && workflowEventsResponse.response.events.length > 0) {
          ...
          for (let i = 0; i < wfEvents.length; i++) {
              ...
              if (wfEvent && wfEvent.data && wfEvent.data.contexts && wfEvent.data.contexts.length > 0) {
                  ...
                  let activity = attributes && attributes.activities && attributes.activities.group && attributes.activities.group.length && attributes.activities.group[0] ? attributes.activities.group[0] : {};
      ...
      for (let workflow in workflowPastEvents) {
          if (workflowPastEvents[workflow].attributes && workflowPastEvents[workflow].attributes.activities && workflowPastEvents[workflow].attributes.activities.group && workflowPastEvents[workflow].attributes.activities.group.length > 0) {
              ...
              for (let i = 0; i < workflowPastEvents[workflow].attributes.activities.group.length; i++) {
      ...
      workflowDefinitionResponse.content.entityModels.length > 0) {
          ...
          for (let i = 0; i < workflowDefinitionResponse.content.entityModels.length; i++) {
              ...
              if (entityModel.data && entityModel.data.attributes) {
                  ...
                  let wfPastAttributes = this._workflowPastEvents[workflowName] && this._workflowPastEvents[workflowName].attributes ? this._workflowPastEvents[workflowName].attributes : {};
