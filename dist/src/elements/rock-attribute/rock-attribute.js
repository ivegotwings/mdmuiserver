...
class RockAttribute extends mixinBehaviors([RUFBehaviors.UIBehavior, RUFBehaviors.ComponentContextBehavior],
  ...
  static get template() {
    return html`
        <style include="bedrock-style-common bedrock-style-scroll-bar bedrock-style-tooltip bedrock-style-floating bedrock-style-icons bedrock-style-padding-margin bedrock-style-list">
            :host(.fallback-value) {
                ...
                <template is="dom-if" if="[[_isComponentEditable(mode, attributeModelObject)]]">
                    <div class\$="[[_getAttributeEditClass(changed)]]">
                        ...
                        <template is="dom-if" if="[[_isDate(attributeModelObject)]]">
                            <pebble-datetime-picker id="datePicker" min-date="[[_convertToDate(attributeModelObject.rangeFrom)]]" max-date="[[_convertToDate(attributeModelObject.rangeTo)]]" label="[[_getLabel(attributeModelObject.externalName)]]" value="{{attributeDisplayValue}}" picker-type="date" no-label-float="[[noLabelFloat]]" description-object="[[_getDescriptionObject()]]" tabindex="[[tabindex]]">
                            </pebble-datetime-picker>
                        </template>
                        <!-- DATETIME -->
                        <template is="dom-if" if="[[_isDateTime(attributeModelObject)]]">
                            <pebble-datetime-picker id="dateTimePicker" min-date="[[_convertToDate(attributeModelObject.rangeFrom)]]" max-date="[[_convertToDate(attributeModelObject.rangeTo)]]" label="[[_getLabel(attributeModelObject.externalName)]]" value="{{attributeDisplayValue}}" picker-type="datetime" no-label-float="[[noLabelFloat]]" description-object="[[_getDescriptionObject()]]" tabindex="[[tabindex]]" default-time="[[_getDefaultTime(attributeObject.value)]]">
                            </pebble-datetime-picker>
                        </template>
                        <!-- TEXTBOX-COLLECTION -->
                ...
                <template is="dom-if" if="[[_isRichTextEditor(attributeModelObject)]]">
                    ...
                    <template is="dom-if" if="[[_isGridType]]">
                        ...
                        <template is="dom-if" if="[[_isEditMode(mode)]]">
                            ...
                            <pebble-dialog id="rteSingleEdit" modal="" show-ok="" button-ok-text="ok" show-close-icon="" no-cancel-on-outside-click="" no-cancel-on-esc-key="" dialog-title="Rich Text Editor">
...
`;
  ...
  _messageTapped() {
      this.$.messagePopover.positionTarget = this.shadowRoot.querySelector('#message-circle');
