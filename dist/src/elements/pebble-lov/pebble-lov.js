...
`;
  ...
  */
  ...
  _onTap(e) {
      ...
      if (!this.multiSelect) {
          this._clearFilter();
          if (!this.showActionButtons && !this.noPopover) {
              this.reset();
          }
      }
  }
  ...
  _patchWheelOverScrolling() {
      ...
      selector.addEventListener('wheel', function (e) {
          let scroller = selector._scroller || selector.scrollTarget;
          let scrolledToTop = scroller.scrollTop === 0;
          let scrolledToBottom = (scroller.scrollHeight - scroller.scrollTop - scroller.clientHeight) <=
              1;
  ...
  _onConfirm(event) {
      ...
      this.dispatchEvent(new CustomEvent("lov-confirm-button-tap", { detail: eventDetail, bubbles: true, composed: false }));
  ...
  _onFavouriteIconTap(e) {
      this._loading = true;
      e.stopPropagation();
      this.dispatchEvent(new CustomEvent("lov-favourite-icon-tap", { detail: e.currentTarget.item, bubbles: true, composed: true }));
  }
