...
class RockRecentActivity extends mixinBehaviors([RUFBehaviors.UIBehavior, RUFBehaviors.ComponentContextBehavior,
  ...
  static get template() {
    return Polymer.html`
    <style include="bedrock-style-common bedrock-style-scroll-bar bedrock-style-grid-layout  bedrock-style-padding-margin bedrock-style-icons">
      :host {
        display: block;
        height: 100%;
        position: relative;
              ...
              <template is="dom-repeat" items="{{items}}" as="item">
                      <div id="pebble_accordion_container">
                          <div class="accordion" on-tap="_onHeaderClick">
                                  <div class="header">
                                      <pebble-icon icon="pebble-icon:action-scope-take-selection" class="pebble-icon-size-12 m-r-5 m-t-10 accordion-icon" id="accordion_icon" on-tap="_onOpenClick"></pebble-icon>
                                      <div class="header-content">
                                          <div class="item-img">
                                                <img src="../src/images/no-photo.jpg">
                                              </div>
                                          <div class="header-text"><pebble-echo-html html="[[_getHeadreText(item)]]"></pebble-echo-html></div>
                                      </div>
                                  </div>
                          </div>
                          <div class="panel">
                                          ...
                                          <div class="item-wrapper">
                                            <div class="item-details group-item-details">
                                              <div class="time-stamp">
                                                ...
                                                <div><pebble-echo-html html="{{newitem.message}}"></pebble-echo-html></div>
                                              </div>
                                            </div>
                                          </div>
  ...
  _onEventsResponse(e) {
    if (e.detail && e.detail.response && e.detail.response.response && e.detail.response.response.entities && e.detail.response.response.entities.length > 0) {
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        eventGroup[i] = {"itemlist":eventHistoryList};
        if(event.entityAdd){
