  ...
  static get template() {
    return html`
        ...
        <template is="dom-if" if="[[_isVisible]]">
            <paper-dialog id="dialog" opened="" on-iron-overlay-opened="_onDialogOpened" on-iron-overlay-closed="_onDialogClose" role="alertdialog" position-target="[[positionTarget]]" horizontal-align="[[horizontalAlign]]" vertical-align="[[verticalAlign]]" dynamic-align="[[dynamicAlign]]" no-overlap="[[noOverlap]]" vertical-offset="[[verticalOffset]]" horizontal-offset="[[horizontalOffset]]" no-cancel-on-outside-click="[[noCancelOnOutsideClick]]" no-cancel-on-esc-key="[[noCancelOnEscKey]]">
                <h2 class="dialogTitle" id="paperDialogTitle" on-track="handleTrack">[[dialogTitle]]
                    <!-- Close Dialog -->
                    <template is="dom-if" if="[[showCloseIcon]]">
                        <div id="closeIcon">
                            <pebble-icon id="close" name="close" icon="pebble-icon:window-action-close" title="Remove" on-tap="_close" class="icon pebble-icon-color-white pebble-icon-size-12" role="button" tabindex="0" aria-disabled="false"></pebble-icon>
                        </div>
                    </template>
                </h2>
