...
class RockGrid
    ...
    static get template() {
        return html`
		<style include="bedrock-style-common bedrock-style-gridsystem bedrock-style-tooltip bedrock-style-grid-layout bedrock-style-icons bedrock-style-padding-margin bedrock-style-buttons bedrock-style-paper-listbox bedrock-style-list">
			:host {
				display: block;
				height: 100%;
			}
												...
												<data-table-column slot="column-slot" name="[[col.header]]" column-index="{{colIndex}}" filter-by="[[_isFilterEnabled(col)]]" sort-by="[[_isSortable(col)]]" sort-type="[[_getColumnSortType(col)]]" data-type="[[_getColumnDataType(col)]]" icon="pebble-icon:action-edit" class="pebble-icon-size-16" column-object="[[col]]" width="[[_getColWidth(col)]]">
													<template>
														...
														<template is="dom-if" if="[[_isNestedAttribute(column.columnObject, item)]]">
															...
															<template is="dom-if" if="[[!_getNestedAttributeValueCount(item, column.columnObject)]]">
																<div slot="cell-slot-content" class="cell tooltip-bottom" data-tooltip$="[[_getEmptyDisplayValue(item)]]">
    ...
    _modeChange(mode, detail) {
            ...
            for (let domRepeat of domRepeats) {
                let item = (domRepeat.parentElement && domRepeat.parentElement.parentElement) && domRepeat.parentElement.parentElement
                    .item;
    ...
    /**
     * Function to handle double-click on the grid row
     **/
    _rowDblClicked(e) {
    ...
    /**
     * Function to handle click on the link of the grid
     **/
    _rowLinkClicked(e) {
            ...
            if (this.config && this.config.itemConfig && this.config.itemConfig.linkTemplate) {
                ...
                this.fireBedrockEvent("grid-link-clicked", { "link": link, "id": item.id }, { ignoreId: true });
